## 使用 datashader 與 mercantile 建立 OpenStreetMap 圖磚系統

- [ipython document](./generate_nyc_map_tiles_blog.ipynb)

- [Blog 文章](https://yeshuanova.github.io/blog/posts/implement-OSM-map-tiles/)

- [互動式地圖範例](https://yeshuanova.github.io/nyc_taxi_trip_map/index.html)

## 使用 datashader 與 mercantile 建立 OpenStreetMap 圖磚系統(Parallel)

- [ipython](./generate_nyc_map_tiles_parallel_blog.ipynb)

- [互動式地圖範例(Parallel)](https://yeshuanova.github.io/nyc_taxi_trip_map/index-parallel.html)
